...
import { Box, Paper, Grid, Typography, Button, Card, Chip } from "@mui/material";
...
export const Profile = () => {
  ...
  const getProfileDetails = async () => {
    try {
      const res = await fetch(
        `${user.role === "Student" ? STUDENT_API_ENDPOINT : TEACHER_API_ENDPOINT}/${user._id}/detail`,
        {
          method: "GET",
          credentials: "include",
        }
  ...
  return (
    ...
    >
      ...
      >
        <Typography variant="h4" align="center" sx={{ color: "#2a5298", mb: 3 }}>
          {profile.name}
        </Typography>
        <Chip
          label={profile.role}
          sx={{ mb: 3, fontWeight: "bold", background: "#49a09d", color: "#fff" }}
        />
